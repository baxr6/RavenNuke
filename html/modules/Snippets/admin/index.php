...
function snippet_menu() {
  global $db, $prefix, $admin_file, $arrSnip_cfg;
  OpenTable();
  ...
  CloseTable();
  OpenTable();
  $tabs = new tabs('snippets');
  $tabs->start(_SNIP_ABOUTSNIP);
  about_snippets();
  $tabs->end();
  $tabs->start(_SNIP_ADDSNIP);
  snippet_add();
  $tabs->end();
  $tabs->start(_SNIP_LISTSNIPPETS);
  snippet_list();
  $tabs->end();
  $tabs->start(_SNIP_ADDMODCAT);
  snippet_cat();
  $tabs->end();
  $tabs->start(_SNIP_STYLE);
  snippet_style();
  $tabs->end();
  $tabs->start(_SNIP_CONFIG);
  snippet_conf();
  $tabs->end();
  $tabs->run();
  CloseTable();
...
<p>
   ...
   <p>
   Snippets is a new <strong>RavenNuke&trade;</strong> module, written by me (Testy1). This is a brand new development. I would greatly appreciate any feedback and feature suggestions for future
<strong>egmade AT gmail DOT com</strong>. </p>
<p>A copy of the GNU General Public License (GPL) is in the root directory of the module archive (GPL.TXT). You may also find more information on the GPL at the <a href="http://www.gnu.org">GNU website</a>.</h1>
<?php
...
function snippet_list() {
  ...
  echo '</table>';
  /*} else {
  echo '<div align="center"><h3>'._SNIP_NOSNIPPETS.'</h3></div>';  
  }*/
  CloseTable();
...
function snippet_cat_save($cat, $cdesc, $cimg) {
  ...
  if ($numrows > 0) {
    ...
    Header("Location: " . $admin_file . ".php?op=snippets#content-snippets-AddModifyCategory");
  }
}
...
function snippet_del_cat($cid, $ok = 0) {
  ...
  if ($ok) {
    ...
    Header('Location: ' . $admin_file . '.php?op=snippet_cat');
  } else {
    include_once ('header.php');
    snippet_menu();
    OpenTable();
...
function snippet_del($id, $ok = 0) {
  ...
  if ($ok) {
    ...
    Header('Location: ' . $admin_file . '.php?op=snippets#content-snippets-ListSnippets');
  } else {
    include_once ('header.php');
    //snippet_menu();
    OpenTable();
...
function snippet_save($title, $code, $cdesc, $tags = null) {
  ...
  if (isset($_POST['code'])) {
    ...
    //$code_output = str_replace( "  ", "&nbsp; ", $code_output );
    //$code_output = str_replace( "  ", " &nbsp;", $code_output );
    //$code_output = str_replace( "\t", "&nbsp; &nbsp;", $code_output );
    //$code_output = preg_replace( "/^ {1}/m", '&nbsp;', $code_output );
...
function snippet_edit_save($id, $title, $code, $desc, $tags, $lang, $cid) {
  ...
  $lang = (int)$_POST['lang'];
  $cid = (int)$cid;
  $db->sql_query('UPDATE ' . $prefix . '_dfw_code SET title=\'' . $title . '\', code=\'' . $code . '\', `desc`=\'' . $desc . '\', tags=\'' . $tags . '\', lang=\'' . $lang . '\', cid=\'' . $cid . '\' WHERE id=\'' .
    $id . '\'');
  Header('Location: ' . $admin_file . '.php?op=snippets#content-snippets-ListSnippets');
}
...
function snippet_conf_save() {
  ...
  Header('Location: ' . $admin_file . '.php?op=snippets#content-snippets-Configuration');
}
...
function snippet_style_save() {
  ...
  Header('Location: ' . $admin_file . '.php?op=snippets#content-snippets-Style');
}
