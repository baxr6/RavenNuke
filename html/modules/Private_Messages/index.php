...
if ( $mode == 'newpm' ) {
	...
	case PRIVMSGS_NEW_MAIL:
		$sql = "user_new_privmsg = GREATEST(user_new_privmsg - 1, 0)";
		break;

	case PRIVMSGS_UNREAD_MAIL:
    $sql = "user_unread_privmsg = CASE 
                WHEN user_unread_privmsg > 0 
                THEN user_unread_privmsg - 1 
                ELSE 0 
            END";
    break;
    ...
    $privmsg_sent_id = isset($privmsg_sent_id) ? $privmsg_sent_id : '';
...
// Initialize the undefined variables before the template assignment
$rank_image = isset($rank_image) ? $rank_image : '';
$poster_joined = isset($poster_joined) ? $poster_joined : '';
$poster_posts = isset($poster_posts) ? $poster_posts : '';
$poster_from = isset($poster_from) ? $poster_from : '';
$poster_avatar = isset($poster_avatar) ? $poster_avatar : '';

// Alternative approach - initialize with default values
...
$template->assign_vars(array(
	...
	if ($submit && $mode == 'edit') {
		...
		if ( $submit ) {
			...
			$privmsg_subject = trim(htmlspecialchars($_POST['subject'], ENT_COMPAT, _CHARSET));
		...
		if ( $submit && !$error ) {
			...
			//
			$to_username = (isset($_POST['username']) ) ? trim(htmlspecialchars(stripslashes($_POST['username']), ENT_COMPAT, _CHARSET)) : '';

			$privmsg_subject = ( isset($_POST['subject']) ) ? trim(htmlspecialchars(stripslashes($_POST['subject']), ENT_COMPAT, _CHARSET)) : '';
