...
else if ( $submit || $refresh || $mode != '' )
{
	...
	if ( $submit )
	{
		...
		$privmsg_subject = trim(htmlspecialchars($_POST['subject'], ENT_COMPAT));
	...
	else if ( $preview || $refresh || $error )
	{
		...
		//
		$to_username = (isset($_POST['username']) ) ? trim(htmlspecialchars(stripslashes($_POST['username'])), ENT_COMPAT) : '';

		$privmsg_subject = ( isset($_POST['subject']) ) ? trim(htmlspecialchars(stripslashes($_POST['subject']), ENT_COMPAT)) : '';
