...
if( isset($_POST['login']) || isset($_GET['login']) || isset($_POST['logout']) || isset($_GET['logout']) )
{
	if( ( isset($_POST['login']) || isset($_GET['login']) ) && (!$userdata['session_logged_in'] || isset($_POST['admin'])) )
	{
		...
		if( $row = $db->sql_fetchrow($result) )
		{
			...
			else
			{
				...
				if ( rn_password_verify($password, $row['user_password']) && $row['user_active'] )
				{
					...
					if( $session_id )
					{
						$url = ( !empty($_POST['redirect']) ) ? str_replace('&amp;', '&', htmlspecialchars($_POST['redirect'], ENT_COMPAT)) : 'index.'.$phpEx;
				...
				elseif( $row['user_active'] )
				{
					...
					$redirect = ( !empty($_POST['redirect']) ) ? str_replace('&amp;', '&', htmlspecialchars($_POST['redirect'], ENT_COMPAT)) : '';
	...
	else if( ( isset($_GET['logout']) || isset($_POST['logout']) ) && $userdata['session_logged_in'] )
	{
		...
		{
			$url = (!empty($_POST['redirect'])) ? htmlspecialchars($_POST['redirect'], ENT_COMPAT) : htmlspecialchars($_GET['redirect'], ENT_COMPAT);
			$url = str_replace('&amp;', '&', $url);
			redirect(append_sid($url, true));
		}
		else
